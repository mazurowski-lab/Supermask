...
class Dense3DSpatialTransformer(nn.Module):
    ...
    def _transform(self, input1, dDepth, dHeight, dWidth):
        ...
        D_mesh = torch.tile(D_mesh.unsqueeze_(0),[batchSize, 1, 1, 1])
        H_mesh = torch.tile(H_mesh.unsqueeze_(0),[batchSize, 1, 1, 1])
        W_mesh = torch.tile(W_mesh.unsqueeze_(0),[batchSize, 1, 1, 1])
    ...
    def _interpolate(self, input, D_upmesh, H_upmesh, W_upmesh):
        ...
        # D_upmesh, H_upmesh, W_upmesh = [D, H, W] -> [BDHW,] #grab 8 nearest corner points for each (x,y,z)
        ...
        #print('rep:',rep.shape,rep.max())
        #print('sss:',(torch.arange(nbatch).float()*imgDpt*imgHgt*imgWdt).unsqueeze_(1))
        bDHW = torch.matmul((torch.arange(nbatch).float()*imgDpt*imgHgt*imgWdt).unsqueeze_(1), rep).cuda().view(-1)
        #print(bDHW)
