...
from .loss import crossCorrelation3D, gradientLoss, DiceSensitivityLoss
...
class cycleMorph(BaseModel):
    ...
    def set_input(self, input):
        ...
        if self.model_type=='mask':
            ...
            self.mask_A.resize_(input_A.size()).copy_(mask_A)
            self.mask_B.resize_(input_B.size()).copy_(mask_B)
    ...
    def backward_G(self):
        ...
        # registration loss for masks
            fake_mask_B, flow_mask_A = self.netG_A(torch.cat([self.real_mask_A, self.real_mask_B], dim=1))
            lossA_mask = self.criterionMk(self.real_mask_B,fake_mask_B)

            fake_mask_A, flow_mask_B = self.netG_B(torch.cat([self.real_mask_B, self.real_mask_A], dim=1))
    ...
    def get_current_visuals(self):
        ...
        ret_visuals = OrderedDict([('real_A', real_A), ('flow_A', flow_A),
                                  ...
                                  ('real_mask_A', real_B), ('real_mask_B', flow_B),
                                  ('fake_mask_A', real_B), ('fake_mask_B', flow_B),
