...
readdata, header = nrrd.read(os.path.join(opt.labelroot,'15_rescale.nrrd'))
labels = np.array([1,2])
...
for isub, dataFile in enumerate(datafiles[startNum:]):
    ...
    readdata1, header1 = nrrd.read(dataFile)
    data = readdata1
    data_vol = data
    data_seg = data
    ...
    vals_regist, _ = dice(regist_seg, label_seg, nargout=2)
    vals_origin, _ = dice(data_seg, label_seg, nargout=2)
    registDice[isub] = vals_regist
    originDice[isub] = vals_origin
    print(np.mean(vals_regist))

    dataName = dataFile.split('/')[-1][:-4]
    savePath = os.path.join(opt.results_dir, 'regist_' + dataName+'mat')
